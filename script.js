...
// In the for loop, add an event listener to each image such that when the mouse is over it, the width goes to 110%
  ...
  for (let x = 0; x <imgArray.length; x ++ ){
    imgArray[x].addEventListener("mouseover", function(event){
       imgArray[x].style.width = "110%"
         })

    imgArray[x].addEventListener("mouseout", function(event){
       imgArray[x].style.width = "100%"
    })
  }
...
// Set up the DOM manipulation flow such that when you click any of the p tags, the font family changes to "Indie Flower", cursive;
  ...
  for (let i = 0; i <pTags.length; i ++ ){
   pTags[i].addEventListener("click", function(event){
     pTags[i].style.fontFamily = "Indie Flower, cursive"
   })
  }
...
for(let i =0; i< buttonsArray.length; i++){
  buttonsArray[i].addEventListener("mouseover", function(event){
    buttonsArray[i].style.backgroundColor = "black"
    buttonsArray[i].style.color = "white"
  })

  buttonsArray[i].addEventListener("mouseout", function(event){
    buttonsArray[i].style.backgroundColor = "light grey"
    buttonsArray[i].style.color = "black"
  })
}
